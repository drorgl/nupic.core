 ...
 * 1. nnzr_ is used as a indicator of whether memory has been allocated for a
 * row
 * or not. If nnzr_[i] == 0, NEVER access ind_[i] or nz_[i], they might not
 * even be initialized.
 * 2. When compact, ind_[0] and nz_[0] point to the memory allocated for the
 * whole
...
#define ITERATE_ON_BOX_COLS                                                    \
  ...
  inline void reAllocateBuffers_(size_type ncols) {
    { // Pre-conditions
      NTA_ASSERT(0 <= ncols) << "SparseMatrix reAllocateBuffers_: "
                             << "Bad ncols: " << ncols;
    } // End pre-conditions
  ...
  inline void erase_(size_type row, size_type *ind_it) {
    { // Pre-conditions
      ...
      NTA_ASSERT(nnzr_[row] > 0) << "SparseMatrix erase_: Empty row #" << row;
    } // End pre-conditions
  ...
  inline size_type index_(size_type row, size_type offset) const {
    { // Pre-conditions
      ...
      NTA_ASSERT(0 <= offset && offset < nnzr_[row])
          << "SparseMatrix index_: "
          << "Invalid offset value: " << offset << " - Should be in [0.."
          << nnzr_[row] << ") "
          << "for row: " << row;
    } // End pre-conditions
   ...
   */
  inline void insertNewNonZero_(size_type i, size_type j, size_type *hint,
...
public:
   ...
   */
  inline SparseMatrix()
      : nrows_(0), nrows_max_(0), ncols_(0), nnzr_(nullptr), ind_mem_(nullptr),
  ...
  /**
   * Copy constructor.
   * The current state is discarded and other is copied. The dimensions and
   * number
   ...
   */
  template <typename InputIterator>
  inline void initializeWithRows(const SparseMatrix &other, InputIterator take,
                                 InputIterator take_end) {
    { // Pre-conditions
   ...
   */
  template <typename InputIterator>
  inline void initializeWithCols(const SparseMatrix &other, InputIterator take,
                                 InputIterator take_end) {
    { // Pre-conditions
  ...
  inline void copy(const SparseMatrix<UI2, R2, I2, RP2, DTZ2> &other) {
    ...
    ITERATE_ON_ALL_ROWS {
      ...
      ind_[row] = indp;
      nz_[row] = nzp;
      std::copy(other.row_nz_index_begin(row), other.row_nz_index_end(row),
                indp);
      std::copy(other.row_nz_value_begin(row), other.row_nz_value_end(row),
                nzp);
      indp += nnz;
      nzp += nnz;
    }
   ...
   *  @li None.
   */
  inline ~SparseMatrix() { deallocate_(); }

  // TESTS

  /**
   * Returns whether this sparse matrix is zero or not. It is zero if it does
   * not
  ...
  inline size_type nBytes() const {
    size_type n = 7 * sizeof(size_type) + 3 * sizeof(value_type) +
  ...
  inline size_type nNonZerosOnRow(size_type row) const {
    ...
    { // Post-conditions
      NTA_ASSERT(0 <= nnzr && nnzr <= nCols())
          ...
          << "post-condition: nnzr = " << nnzr << " when ncols = " << nCols();
    } // End post-conditions
  ...
  inline size_type nNonZerosOnCol(size_type col) const {
    ...
    { // Post-conditions
      NTA_ASSERT(0 <= nnzc && nnzc <= nRows())
          ...
          << "post-condition: nnzc = " << nnzc << " when nrows = " << nRows();
    } // End post-conditions
  ...
  inline size_type nNonZeros() const {
    ...
    { // Post-conditions
      NTA_ASSERT(0 <= nnz && nnz <= nRows() * nCols())
          ...
          << " when nrows = " << nRows() << " and ncols = " << nCols();
    } // End post-conditions
   ...
   *
   * @param it [OutputIterator] an iterator to storage for the number of
   * non-zeros
  ...
  /**
   * Returns the number of non-zeros for each each specified row on the
   * specified columns.
   *
   * @param rows
   * The rows to query
  ...
  /**
   * Returns the number of columns that are not zero, i.e. that have at least
   * one
   * non-zero.
   *
  ...
  /**
   * Returns the number of zero rows, i.e. the number of rows that don't have
   * any
   * non-zero.
   *
   ...
   *  @li None.
   */
  inline size_type nZeroRows() const { return nRows() - nNonZeroRows(); }
   ...
   *  @li None.
   */
  inline size_type nZeroCols() const { return nCols() - nNonZeroCols(); }
   ...
   *  @li If row < 0 || row >= nrows (assert)
   */
  inline std::pair<size_type, value_type>
  ...
  /**
   * Returns the number of columns between the first and the last non-zero on
   * the
   * given row.
   *
   ...
   */
  inline std::pair<size_type, value_type>
  firstNonZeroOnCol(size_type col) const {
    { // Pre-conditions
   ...
   */
  inline std::pair<size_type, value_type>
  lastNonZeroOnCol(size_type col) const {
    { // Pre-conditions
   ...
   * [col_begin, col_end).
   */
  inline bool nonZerosInRowRange(size_type row, size_type col_begin,
   ...
   */
  inline size_type nNonZerosInRowRange(size_type row, size_type col_begin,
                                       size_type col_end) const {
    { // Pre-conditions
  ...
  /**
   * Returns the number of non-zeros in a box defined by the indices of the
   * first
   * and one past the last row, and the first and one past the last column, i.e.
   * the box is: [row_begin, row_end) X [col_begin, col_end).
   *
   * @param row_begin [0 <= size_type <= row_end] the first row of the box to
   * look into
   * @param row_end [row_begin <= size_type <= nrows] one past the end of the
   * last row
   *  of the box to look into
   * @param col_begin [0 <= size_type <= col_end] the first col of the box to
   * look into
   * @param col_end [col_begin <= size_type <= ncols] one past the end of the
   * last col
   *  of the box to look into
   ...
   */
  inline size_type nNonZerosInBox(size_type row_begin, size_type row_end,
                                  size_type col_begin,
                                  size_type col_end) const {
    { // Pre-conditions
   ...
   * and a set methods.
   *
   * @param row_inds_begin [InputIterator] iterator to beginning of range that
   * contains
   *  the row indices to use
   * @param row_inds_end [InputIterator] iterator to one past the end of the
   * range
   *  that contain the row indices to use
   * @param col_inds_begin [InputIterator] iterator to beginning of range that
   * contains
   *  the col indices to use
   * @param col_inds_end [InputIterator] iterator to one past the end of the
   * range
   *  that contain the col indices to use
   * @param summary [Summary&] storage for the result, must have resize and set
   * methods
   *
   * @b Complexity:
   *  @li O(2 * (row_end - row_begin) * log(nnzr))
  ...
  nNonZerosPerBox(InputIterator row_inds_begin, InputIterator row_inds_end,
    ...
    for (InputIterator row = row_inds_begin; row != row_inds_end;
         ++row, ++box_i) {
      size_type prev_col = 0, box_j = 0;
      for (InputIterator col = col_inds_begin; col != col_inds_end;
           ++col, ++box_j) {
        value_type nnzib =
            (value_type)nNonZerosInBox(prev_row, *row, prev_col, *col);
  ...
  /**
   * Returns whether this matrix is binary or not (the non-zeros are all the
   * same
   ...
   *
   * @param it [OutputIterator] iterator to the beginning of the range that will
   *  receive the indices of the non-zero rows. It will iterate at most nRows()
   * times.
   *
   * @b Complexity:
   *  @li O(nrows)
   ...
   *
   * @param it [OutputIterator] iterator to the beginning of the range that will
   *  receive the indices of the zero rows. It will iterate at most nRows()
   * times.
   *
   * @b Complexity:
   *  @li O(nrows)
   ...
   *
   * @param it [OutputIterator] iterator to the beginning of the range that will
   *  receive the indices of the non-zero columns. It will iterate at most
   * nCols() times.
   *
   * @b Complexity:
   *  @li O(ncols)
   ...
   *
   * @param it [OutputIterator] iterator to the beginning of the range that will
   *  receive the indices of the zero columns. It will iterate at most nCols()
   * times.
   *
   * @b Complexity:
   *  @li O(ncols)
   ...
   * two sparse matrices.
   */
  inline bool sameRowNonZeroIndices(size_type row,
                                    const SparseMatrix &B) const {
    {
      ...
      NTA_ASSERT(0 <= row && row < B.nRows())
          << "SparseMatrix::sameRowNonZeroIndices: "
          << "Invalid row index: " << row << " - B matrix has only: " << nRows()
          << " rows";
    }
  ...
  /**
   * Checks whether two sparse matrices have all their zeros in the same
   * locations.
   */
  ...
  /**
   * Checks whether the indices of the non-zeros on row of this SM are a subset
   * of the
   ...
   * non-zeros of B.
   */
  inline bool nonZeroIndicesIncluded(size_type row,
                                     const SparseMatrix &B) const {
    { // Pre-conditions
      NTA_ASSERT(0 <= row && row < nRows())
          << "SparseMatrix::sameRowNonZeroIndices: "
  ...
  /**
   * Checks whether the locations of the non-zeros of this sparse matrix are
   * included
  ...
  inline std::istream &fromCSR(std::istream &inStreamParam,
    ...
    { // Pre-conditions
      NTA_CHECK(inStreamParam.good()) << where << "Bad stream";
    } // End pre-conditions

    std::string tag;
    inStreamParam >> tag;
    NTA_CHECK(tag == "csr" || tag == "sm_csr_1.5")
        << where << "Stream is not in csr format"
    ...
    {
      NTA_CHECK(nrows >= 0) << where << "Invalid number of rows: " << nrows
                            << " - Should be >= 0";

      NTA_CHECK(ncols >= 0) << where << "Invalid number of columns: " << ncols
                            << " - Should be > 0";

      NTA_CHECK(nnz >= 0 && (double)nnz <= (double)nrows * ncols)
          << where << "Invalid number of non-zeros: " << nnz
    ...
    for (i = 0; i != nrows; ++i) {
...

      {
        NTA_CHECK(nnzr >= 0 && nnzr <= ncols)
            << where << "Invalid number of non-zeros: " << nnzr
   ...
   *  nnzr2 ...
   *
   * where totalbytes is the total number of bytes that follow in the csr
   * description
  ...
  inline std::ostream &toCSR(std::ostream &out) const {
    { // Pre-conditions
      NTA_CHECK(out.good()) << "SparseMatrix::toCSR(): Bad stream";
    } // End pre-conditions
  ...
  inline void fromBinary(std::istream &inStream) {
    ...
    { // Pre-conditions
      NTA_CHECK(inStream.good()) << "SparseMatrix::fromBinary: Bad stream";
    } // End pre-conditions
    ...
    {
      NTA_CHECK(s1 == sizeof(size_type)) << where << "Bad size_type: " << s1;
...

    {
      ...
      NTA_CHECK(0 <= ncols) << where << "Bad number of columns: " << ncols;
      NTA_CHECK(0 <= nnz) << where << "Bad number of non-zeros: " << nnz;
    }
  ...
  inline void toBinary(std::ostream &outStream) {
    ...
    { // Pre-conditions
      NTA_CHECK(outStream.good()) << "SparseMatrix::toBinary: Bad stream";
    } // End pre-conditions
    ...
    outStream << "sm_bin_1.5 " << (int)isSystemLittleEndian() << ' '
              << sizeof(size_type) << ' ' << sizeof(value_type) << ' '
              << sizeof(difference_type) << ' ' << sizeof(prec_value_type)
  ...
  /**
   * Deletes the row at the index specified. The resulting matrix has one less
   * row.
   */
   ...
   *
   * @param del [InputIterator<size_type>] iterator to the beginning of the
   * range
   *  that contains the indices of the rows to be deleted
   * @param del_end [InputIterator<size_type>] iterator to one past the end of
   * the
   ...
   * @param ind [InputIterator1<size_type>] input iterator for indices of
   * non-zeros
   * @param nz [InputIterator2<value_type>] input iterator for values of
   * non-zeros
   *
   * @b Exceptions:
   *  @li Not enough memory (error)
   ...
   * @param resizeYesNo [bool] whether to increase the size of this matrix or
   * not
   *
   * @b Complexity:
   *  @li Worst case: O(nrows + log(nnzr) + nnz + 4*nnzr) (if decompact in
   * resize)
   *
   * @b Exceptions:
        ...
        if (i_begin != i_end) {
          ...
          for (; ii != i_end; ++ii, ++jj, ++vv) {
            NTA_ASSERT(*iip < *ii || *jjp < *jj)
                << where
                << "Passed in duplicate or out-of-order non-zeros: " << *vvp
                << " and " << *vv << ", (" << *iip << ", " << *jjp << ") and ("
                << *ii << ", " << *jj << ")";
            iip = ii;
            jjp = jj;
  ...
  inline void getSlice(size_type src_first_row, size_type src_row_end,
    ...
    { // Pre-conditions
      ...
      NTA_ASSERT(src_first_row <= src_row_end)
          ...
          << ": Invalid row range: [" << src_first_row << ".." << src_row_end
          << "): "
          << "- Beginning should be <= end of range";
      NTA_ASSERT(src_first_col <= src_col_end)
          << "SparseMatrix getSlice"
          << ": Invalid column range: [" << src_first_col << ".." << src_col_end
          << "): "
          << "- Beginning should be <= end of range";
    } // End pre-conditions
   ...
   * this = outer(x,y) .* b
   */
  inline void setFromElementMultiplyWithOuter(const std::vector<value_type> &x,
                                              const std::vector<value_type> &y,
   ...
   *
   * @param row [0 <= size_type < nrows] the row to set
   * @param ind_it [InputIterator<size_type>] iterator to the beginning of a
   * range
   ...
   * @param nz_it [InputIterator<value_type>] iterator to the beginning of the
   * range
   ...
   * @param idxValIt [OutputIterator1] an iterator pointing to beginning of
   * container
  ...
  inline void setColFromDense(size_type col, const std::vector<value_type> &x) {
    { // Pre-conditions
      NTA_ASSERT(x.size() == nRows()) << "SparseMatrix setColFromDense std: "
    ...
    if (n > 0) {
      ...
      for (size_type row = 0; row != nrows; ++row) {
        ...
        const size_type *ind_end = std::lower_bound(
            ind_begin, const_cast<const size_type *>(ind_end_(row)), max);
   ...
   *
   * @b Complexity:
   *  @li Worst case: O(nnz in box + 2 * n box rows * log(nnzr)) (if f1(0) ==
   * true)
   ...
   */
  template <typename BinaryFunction>
  inline void rowExtremumNZ(size_type row, size_type &idx, value_type &ext_val,
   ...
   * @retval [std::pair<size_type,value_type>] the position and value of the
   * maximum
   *  for row row_index
   *
   * @b Exceptions:
   ...
   * There are as many pairs in that array as there are rows in the sparse
   * matrix.
   * The first element in each pair is the column index of the maximum for each
   * row,
   * and the second element is the value of that maximum.
   *
   * @param maxima [OutputIterator] the vector of the row-wise maxima (index,
   * value)
   *
   * @b Exceptions:
   *  @li None
   ...
   * There are as many pairs in that array as there are rows in the sparse
   * matrix.
   * The first element in each pair is the column index of the minimum for each
   * row,
   * and the second element is the value of that minimum.
   *
   * @param minima [OutputIterator] the vector of the row-wise minima (index,
   * value)
   *
   * @b Exceptions:
   *  @li None
   ...
   */
  template <typename BinaryFunction>
  inline void colExtremumNZ(size_type col, size_type &idx, value_type &ext_val,
                            const BinaryFunction &f2) const {
    idx = 0;
  ...
  inline void boxMin(size_type begin_row, size_type end_row,
                     size_type begin_col, size_type end_col, size_type &min_row,
                     size_type &min_col, value_type &min_val) const {
    { // Pre-conditions
  ...
  inline void boxMax(size_type begin_row, size_type end_row,
                     size_type begin_col, size_type end_col, size_type &max_row,
                     size_type &max_col, value_type &max_val) const {
    { // Pre-conditions
   ...
   * the threshold used to determine whether a number is a zero or not.
   *
   * @param val [value_type != 0] the value that should be used in the
   * normalization
   * @param exact [bool (false)]: whether to make the rows exactly sum to val
   *  after normalization or not
   *
  ...
  inline void rowSums(std::vector<value_type> &sums) const {
    { // Pre-conditions
      NTA_ASSERT(sums.size() == nRows()) << "rowSums: Wrong size for vector";
    } // End pre-conditions
   ...
   *
   *  prods[row] = prod(this[row,col], for col in [0,ncols), s.t. this[row,col]
   * != 0)
   *
   * where:
   *  row in [0,nrows)
   ...
   *
   *  prods[col] = prod(this[row,col], for row in [0,nrows), s.t. this[row,col]
   * != 0)
   *
   * where:
   *  col in [0,ncols)
   *
   * @param prods [OutputIterator] iterator to storage for the prod of each
   * column
   *
   * @b Exceptions:
   *  @li None.
   ...
   * @param x [InputIterator<value_type>] input vector, a contiguous array in
   * memory,
  ...
  inline void multiply(const SparseMatrix &B, SparseMatrix &C) const {
    { // Pre-conditions
      NTA_ASSERT(nCols() == B.nRows())
          ...
          << "should be the same as B matrix's number of rows (" << B.nRows()
          << ")";
    } // End pre-conditions
   ...
   *    y[row,block] = sum(this[row,col] * x[col], for col in [block_begin,
   * block_end))
   *
   * @param x [InputIterator<value_type>] input vector (size = number of
   * columns)
   * @param y [OutputIterator<value_type>] result (size = number of rows X
   * number
   *  of blocks)
   *
   * @b Exceptions:
  ...
  /**
   * Left vec prod of x and this matrix, putting the result into y, where x and
   * y
  ...
  inline void rightVecMaxAtNZ(InputIterator x, OutputIterator y) const {
    ITERATE_ON_ALL_ROWS {
      ...
      *y++ = max_val != -std::numeric_limits<value_type>::max() ? max_val
                                                                : (value_type)0;
    }
  }
   ...
   *
   *  for row in [0,nrows):
   *   y[row] = max(lb, prod(x[col], for col in [0,ncols) s.t. this[row,col] !=
   * 0))
   *
   * @param x [InputIterator<value_type>] input vector (size = number of
   * columns)
   ...
   *  for row in [0,nrows):
   *   y[row] =
   *    prod(this[row,col] * x[col], for col in [0,ncols) s.t. this[row,col] !=
   * 0)
   *
   * @param x [InputIterator<value_type>] input vector (size = number or
   * columns)
   * @param y [OutputIterator<value_type>] output vector (size = number of rows)
   *
   * @b Exceptions:
   ...
   *  for row in [0,nrows):
   *   y[row] =
   *    max(this[row,col] * x[col], for col in [0,ncols) s.t. this[row,col] !=
   * 0)
   *
   * @param x [InputIterator<value_type>] input vector (size = number of
   * columns)
   ...
   *  for row in [0,nrows):
   *   y[row] =
   *    argmax(this[row,col] * x[col], for col in [0,ncols) s.t. this[row,col]
   * != 0)
   *
   * @param x [InputIterator<value_type>] input vector (size = number of
   * columns)
 ...
 */
template <typename I, typename F, typename I2, typename F2, typename ZeroTest>
inline bool operator==(const SparseMatrix<I, F, I2, F2, ZeroTest> &A,
